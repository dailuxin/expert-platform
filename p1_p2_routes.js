...
router.get('/announcements', (req, res) => {
  const db = req.app.get('db');
  const rows = db.query("SELECT * FROM announcements ORDER BY created_at DESC LIMIT 20");
  res.json(rows);
});
...
router.post('/announcements', (req, res) => {
  ...
  db.run("INSERT INTO announcements (title, content, created_at) VALUES (?, ?, datetime('now'))", [title, content]);
  db.save();
...
router.get('/admin/logs', (req, res) => {
  if (!req.session.user || req.session.user.role !== 'admin') return res.status(403).json({ error: '无权限' });
  const db = req.app.get('db');
  const rows = db.query("SELECT * FROM operation_logs ORDER BY created_at DESC LIMIT 200");
  res.json(rows);
});
...
router.get('/admin/export/users', (req, res) => {
  if (!req.session.user || req.session.user.role !== 'admin') return res.status(403).json({ error: '无权限' });
  const db = req.app.get('db');
  const rows = db.query("SELECT id, name, email, role, status, industry, created_at FROM users ORDER BY created_at DESC");
...
router.post('/admin/backup', (req, res) => {
  ...
  res.json({ success: true, filename, size: data.length });
});
