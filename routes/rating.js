// routes/rating.js — 专家评分 + 排名路由
// 调用方式：require('./routes/rating.js')(app, requireAuth, requireExpert);
// 必须在 app.listen() 之前调用

const db = require('../database');

module.exports = function(app, requireAuth, requireExpert) {

  // === 测试路由：验证本文件是否加载成功 ===
  app.get('/api/rating-test', (req, res) => {
    res.json({ ok: true, msg: 'rating routes loaded' });
  });
  ...
  app.post('/api/experts/:id/reviews', requireAuth, (req, res) => {
    ...
    if (!rating || rating < 1 || rating > 5) {
      return res.json({ success: false, message: '评分必须在 1-5 之间' });
    }
  ...
  app.get('/api/experts/:id/reviews', (req, res) => {
    const list = db.query(
      'SELECT r.id, r.rating, r.content, r.reply, r.replied_at, r.created_at, u.real_name as user_name ' +
      'FROM reviews r JOIN users u ON u.id = r.user_id ' +
      'WHERE r.expert_id = ? ORDER BY r.created_at DESC LIMIT 100',
      [req.params.id]
    );
  ...
  app.put('/api/reviews/:id/reply', requireAuth, requireExpert, (req, res) => {
    ...
    const review = db.get(
      'SELECT r.*, e.id as eid FROM reviews r JOIN experts e ON e.id = r.expert_id WHERE r.id = ?',
      [req.params.id]
    );
    if (!review) return res.json({ success: false, message: '评价不存在' });
    if (review.eid !== req.expert.id) return res.json({ success: false, message: '只能回复自己的评价' });
