...
app.post('/api/experts/:id/reviews', requireAuth, (req, res) => {
  ...
  if (existing) return res.json({ success: false, message: '您已评价过该专家，如需修改请联系管理员' });
  db.run(
...
app.get('/api/experts/:id/reviews', (req, res) => {
  const list = db.query(\`
    SELECT r.id, r.rating, r.content, r.created_at, u.real_name as user_name
    FROM reviews r
...
app.put('/api/reviews/:id/reply', requireAuth, requireExpert, (req, res) => {
  ...
  const review = db.get('SELECT r.*, e.user_id FROM reviews r JOIN experts e ON e.id = r.expert_id WHERE r.id = ?', [req.params.id]);
  if (!review) return res.json({ success: false, message: '评价不存在' });
  if (review.user_id !== req.expert.id) return res.json({ success: false, message: '只能回复自己的评价' });
...
app.get('/api/experts/ranking', (req, res) => {
  const { sort = 'rating', order = 'desc', limit = 20 } = req.query;
  const allowed = { rating: 'avg_rating', reviews: 'review_count', income: '(SELECT total_income FROM expert_wallet WHERE expert_id = experts.id)', consultations: '(SELECT COUNT(*) FROM bookings WHERE expert_id = experts.id AND status = "completed")' };
  const col = allowed[sort] || 'avg_rating';
  const dir = order === 'asc' ? 'ASC' : 'DESC';
  const list = db.query(\`
    SELECT e.id, e.industry, e.title, e.avg_rating, e.review_count, e.views,
           u.real_name,
           (SELECT total_income FROM expert_wallet WHERE expert_id = e.id) as total_income,
           (SELECT COUNT(*) FROM bookings WHERE expert_id = e.id AND status = 'completed') as completed_consultations
    FROM experts e
